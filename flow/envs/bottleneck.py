...
class BottleneckAccelEnv(BottleneckEnv):
    ...
    def compute_reward(self, rl_actions, **kwargs):
        ...
        v_top = self.max_speed
        return np.mean(vel) / v_top
...
class BottleneckDesiredVelocityEnv(BottleneckEnv):
    ...
    def __init__(self, env_params, sim_params, network, simulator='traci'):
        ...
        self.segment_list = [segment[1] for segment in self.segments if segment[2]]
        self.total_controlled_segments = int(np.sum(self.segment_list))
    ...
    def observation_space(self):
        ...
        for segment in self.segments:
            num_obs += 4 * self.k.network.num_lanes(segment[0]) * segment[1] * segment[2]
    ...
    def get_state(self):
        ...
        * The average speed of the rl vehicles in that segment.
        ...
        for i, edge in enumerate(self.controlled_edges):
            num_lanes = self.k.network.num_lanes(edge)

            num_vehicles = np.zeros((self.segment_list[i], num_lanes))
            num_rl_vehicles = np.zeros((self.segment_list[i], num_lanes))
            vehicle_speeds = np.zeros((self.segment_list[i], num_lanes))
            rl_vehicle_speeds = np.zeros((self.segment_list[i], num_lanes))

            ids = self.k.vehicle.get_ids_by_edge(edge)
            lane_list = self.k.vehicle.get_lane(ids)
            pos_list = self.k.vehicle.get_position(ids)

            for j, veh in enumerate(ids):
                segment = np.searchsorted(self.slices[edge], pos_list[j]) - 1
                if veh in self.k.vehicle.get_rl_ids():
                    rl_vehicle_speeds[segment, lane_list[j]] += self.k.vehicle.get_speed(veh)
                    num_rl_vehicles[segment, lane_list[j]] += 1
                else:
                    vehicle_speeds[segment, lane_list[j]] += self.k.vehicle.get_speed(veh)
                    num_vehicles[segment, lane_list[j]] += 1
        ...
        for i in range(len(num_rl_vehicles_list)):
            if num_vehicles_list[i] != 0:
                vehicle_speeds_list[i] /= num_vehicles_list[i]
            if num_rl_vehicles_list[i] != 0:
                rl_speeds_list[i] /= num_rl_vehicles_list[i]

            num_vehicles_list[i] /= NUM_VEHICLE_NORM
            num_rl_vehicles_list[i] /= NUM_VEHICLE_NORM

            vehicle_speeds_list[i] /= 50
            rl_speeds_list[i] /= 50

        return np.concatenate((num_vehicles_list, num_rl_vehicles_list,
                               vehicle_speeds_list, rl_speeds_list))
            ...
            if edge:
                ...
                if edge[0] != ':' and edge in self.controlled_edges:
                    ...
                    if not self.symmetric:
                        ...
                        action = rl_actions[int(lane) + bucket * num_lanes + self.action_index[edge][0]]
                    else:
                        # find what segment we fall into
                        bucket = np.searchsorted(self.slices[edge], pos) - 1
                        action = rl_actions[bucket + self.action_index[edge][0]]
    ...
    def compute_reward(self, rl_actions, **kwargs):
        """Outflow rate over last ten seconds normalized to max of 1."""
        '''vel = np.array(self.k.vehicle.get_speed(self.k.vehicle.get_ids()))
        v_top = self.k.network.max_speed()
        return np.mean(vel) / v_top'''
...
class BottleneckDesiredVelocityEnv2(BottleneckEnv):
    ...
    def __init__(self, env_params, sim_params, network, simulator='traci'):
        ...
        add_env_params = self.env_params.additional_params
        default = [(str(i), 1, True) for i in range(1, 6)]
        super(BottleneckDesiredVelocityEnv2, self).__init__(env_params, sim_params, network)
        self.segments = add_env_params.get("controlled_segments", default)
    ...
    def compute_reward(self, rl_actions, **kwargs):
        ...
        for i in range(len(num_vehicles_list)):
            if num_vehicles_list[i] != 0:
                vehicle_speeds_list[i] /= num_vehicles_list[i]

            vehicle_speeds_list[i] /= self.k.network.max_speed()

        vel = np.array(self.k.vehicle.get_speed(self.k.vehicle.get_ids()))
        v_top = self.k.network.max_speed()
        vehicle_speeds_list.append(np.mean(vel) / v_top)

        return vehicle_speeds_list

...
class MergePOEnv(Env):
    ...
    * num_rl: maximum number of controllable vehicles in the network
    ...
    def get_state(self, rl_id=None, **kwargs):
        ...
        for i, rl_id in enumerate(self.rl_veh):
            this_position = self.k.vehicle.get_x_by_id(rl_id)
            ...
            observation[6 * i + 0] = this_speed / max_speed
            observation[6 * i + 1] = (lead_speed - this_speed) / max_speed
            observation[6 * i + 2] = lead_head / max_length
            observation[6 * i + 3] = (this_speed - follow_speed) / max_speed
            observation[6 * i + 4] = follow_head / max_length
            observation[6 * i + 5] = this_position / max_length
    ...
    def compute_reward(self, rl_actions, **kwargs):
        ...
        return np.mean(self.k.vehicle.get_speed(self.k.vehicle.get_ids())) / self.k.network.max_speed()

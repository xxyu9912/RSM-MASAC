...
class TrafficLightGridPOEnv(TrafficLightGridEnv):
    ...
    * num_observed: number of vehicles nearest each intersection that is
    ...
    def get_state(self):
        ...
        all_observed_ids = []
        mapping = self.network.node_mapping
        for i in range(self.num_traffic_lights):
            ...
            for edge in edges:
                ...
                if len(ids) > 0:
                    ...
                    density += [0]
                    velocity_avg += [0.99]
            individual_state = np.concatenate([density, velocity_avg,
                                               [self.last_change.flatten()[i]],
                                               [self.direction.flatten()[i]],
                                               [self.currently_yellow.flatten()[i]]])
            state.append(individual_state)
    ...
    def compute_reward(self, rl_actions, **kwargs):
        ...
        for i in range(self.num_traffic_lights):
            ...
            for edge in edges:
                ids = self.k.vehicle.get_ids_by_edge(edge)
                for element in ids:
                    observed_ids.append(element)
            speed += [self.k.vehicle.get_speed(veh_id) / v_top for veh_id in observed_ids]
...
class TrafficLightGridPOEnv2(TrafficLightGridEnv):
    ...
    def compute_reward(self, rl_actions, **kwargs):
        ...
        for i in range(self.num_traffic_lights):
            ...
            for edge in edges:
                ids = self.k.vehicle.get_ids_by_edge(edge)
                if len(ids) > 0:
                    speed += np.mean([self.k.vehicle.get_speed(veh_id) for veh_id in ids]) / max_speed
                else:
                    speed += 0.0
            speeds.append(speed / len(edges))

        vel = np.array(self.k.vehicle.get_speed(self.k.vehicle.get_ids()))
        v_top = max(
            self.k.network.speed_limit(edge)
            for edge in self.k.network.get_edge_list())
        speeds.append(np.mean(vel) / v_top)

...
def min_delay_unscaled(env):
    ...
    float
        reward value
    """
    num_traffic_lights = 25
    v_top = max(
        env.k.network.speed_limit(edge)
        for edge in env.k.network.get_edge_list())
    reward = []
    mapping = env.network.node_mapping
    for i in range(num_traffic_lights):
        ...
        for edge in edges:
            ...
            observed_ids.append(x for x in ids)
        speed += [env.k.vehicle.get_speed(veh_id) / v_top for veh_id in observed_ids]
